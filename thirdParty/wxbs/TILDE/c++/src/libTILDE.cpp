...
vector < Mat > getLuv_fast(const Mat & input_color_image)
{
  ...
  cv::parallel_for_( cv::Range (0, input_color_image.rows), [=] (const cv::Range &r)
  {

      Rect roi(0, r.start, input_color_image.cols, r.end - r.start);
...
vector < Mat > getGrad_fast(const Mat & input_color_image)
{
  ...
  // Get Max idx using Magnitude
  cv::parallel_for_( cv::Range (0, gx[0].rows), [=] (const cv::Range &r)
  {
      vector<Mat> inx(3);
      Rect roi(0, r.start, gx[0].cols, r.end - r.start);
      inx[0] = Mat(gx[0], roi);
      inx[1] = Mat(gx[1], roi);
      inx[2] = Mat(gx[2], roi);
